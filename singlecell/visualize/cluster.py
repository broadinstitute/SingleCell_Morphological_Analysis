...
def add_clustering_index_column(sc_df, cp_features_analysis, clustering_method, n_clusters):
    
    if clustering_method=='kmeans':
        cls_model = KMeans(n_clusters=n_clusters)#, random_state=0)
    
    elif clustering_method=='spectral':
        cls_model = cluster.SpectralClustering(n_clusters=n_clusters,affinity="nearest_neighbors")
    ...
    distance_to_cluster_cent=cls_model.fit_transform(sc_df[cp_features_analysis].values)
...
def subpopulation_clustering_visualization(sc_df,cp_features_analysis,params,pooled=False):
    """ 
...
#     print(hist_pert, bin_edges)
    sns.distplot(sc_pert,kde=False,norm_hist=True,bins=bin_edges,label=d,ax=axes[0],\
                 color="r",hist_kws=dict(edgecolor="k"));
    sns.distplot(sc_control,kde=False,norm_hist=True,bins=bin_edges,label=contLabel,ax=axes[0],\
                 hist_kws=dict(edgecolor="k"))
    sns.distplot(sc_pert,kde=False,hist=True,norm_hist=False,bins=bin_edges,label=d,ax=axes[1],\
                 color="r",hist_kws=dict(edgecolor="k"));
    sns.distplot(sc_control,kde=False,hist=True,norm_hist=False,bins=bin_edges,label=contLabel,ax=axes[1],\
                 hist_kws=dict(edgecolor="k"))
...
#   axes.xaxis.set_ticklabels(range(0,20,2)); 
    ...
    for c in range(len(hist_pert)):
        if abs(histDiff[c])>diff_threshold and (hist_pert[c] > 0.001 or hist_control[c] > 0.001):
    #         c=3;
#             print(histDiff[c])
            clusterDF=sc_df[sc_df['clusterLabels2'] == c].reset_index(drop=True)
            mean_pert_cluster=clusterDF.loc[~(clusterDF['label'] == contLabel),cp_features_analysis].mean();        
            diffOfMutMeanAndWTMean=pd.DataFrame(data=mean_pert_cluster.values-mean_control.values,\
                                                columns=['Diff'],index=mean_pert_cluster.index);
...
#             nSampleSCs=6
            if clusterDF.shape[0]> nSampleSCs:
                samples2plot=clusterDF.sort_values('distance_2cluster_center',ascending=True).\
                sample(nSampleSCs).reset_index(drop=True)
                title_str="Cluster "+str(c)
                if pooled:
                    im_size=5500
                    f=visualize_n_SingleCell.visualize_n_SingleCell_pooled(channels,samples2plot,\
                                                                           boxSize,im_size,title=title_str);
                else:
                    f=visualize_n_SingleCell.visualize_n_SingleCell(channels,samples2plot,boxSize,\
                                                                    info_columns=['label'],\
                                                                    title=title_str,compressed=compressed_ims_flag,\
...
#################################
def subpopulation_triplet_clustering_visualization(sc_df,cp_features_analysis,params,pooled=False):
...
#     print(histDiff)
    ...
    sns.distplot(data2plotTR,kde=False,norm_hist=True,bins=bin_edges,label=tr,ax=axes[0],color="g",hist_kws=dict(edgecolor="k"))
    ...
    sns.distplot(data2plotTR,kde=False,hist=True,norm_hist=False,bins=bin_edges,label=tr,ax=axes[1],color="g",hist_kws=dict(edgecolor="k"))
...
#     meanWT=wtANDmtDf_scaled.loc[wtANDmtDf_scaled['label'] == contLabel,feats2use].mean()
    ...
    #             meanMutCluster=clusterDF.loc[~(clusterDF['label'] == contLabel),feats2use].mean();     
                meanNonMutCluster=clusterDF.loc[~(clusterDF['label'] == d),cp_features_analysis].mean();   
    #             diffOfMutMeanAndWTMean=pd.DataFrame(data=meanMutCluster.values-meanWT.values,columns=['Diff'],index=meanMutCluster.index);
                diffOfMutMeanAndWTMean=pd.DataFrame(data=meanNonMutCluster.values-meanMT.values,columns=['Diff'],index=meanNonMutCluster.index);
                diffOfMutMeanAndWTMean.loc[:,'Diff2']=diffOfMutMeanAndWTMean.loc[:,'Diff'].abs()
                absFeatureImportanceSS=diffOfMutMeanAndWTMean.sort_values('Diff2',ascending=False)[:10];
                fig, axes = plt.subplots()
                sns.barplot(x='Diff', y=absFeatureImportanceSS.index, data=absFeatureImportanceSS,ax=axes)
...
#   axes.xaxis.set_ticklabels(range(0,20,2)); 
    ...
    #         c=3;
            ...
            if clusterDF.shape[0]> nSampleSCs:
                ...
                if pooled:
                    im_size=5500
                    f=visualize_n_SingleCell.visualize_n_SingleCell_pooled(compartments,samples2plot,boxSize,im_size,title=title_str);
                else:
                    f=visualize_n_SingleCell.visualize_n_SingleCell(compartments,samples2plot,boxSize,title=title_str)
                f.savefig(resultsDir+'/cluster'+str(c)+'_examplar'+saveFormat)     
                plt.close('all')    
