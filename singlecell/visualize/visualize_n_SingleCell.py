        ...
        for c in channels:
            if c == "Outline":
                ...
                if os.path.exists(imPath):
                    ...
                    imD2 = resize(
                        read_image_from_gallery(imPath),
                        (im_size, im_size),
                        mode="constant",
                        preserve_range=True,
                        order=0,
...
def visualize_n_SingleCell(
    ...
    if compressed:
        metadata_cols_4size = sc_df.columns[
            sc_df.columns.str.contains("Width|ImageSize")
        ]
        if len(metadata_cols_4size):
            original_im_size = sc_df[metadata_cols_4size[0]].values[0]
        else:
            raise Exception(
                "No metadata columns for inferring image size are detected! Please enter original_im_size here!"
            )
        ...
        sc_df["Nuclei_Location_Center_X"] = (
            sc_df["Nuclei_Location_Center_X"] * compRatio
        )
        sc_df["Nuclei_Location_Center_Y"] = (
            sc_df["Nuclei_Location_Center_Y"] * compRatio
        )
    ...
    for index in range(rows_count):
        ...
        for ci in range(len(channels)):
            ...
            try:
                ...
                image_cropped = crop_cell.crop_single_cell_image(
                    image, xCenter, yCenter, halfBoxSize
                )
                #             print(image_cropped.shape,xCenter,yCenter)

                if 1:
                    image_cropped = exposure.rescale_intensity(
                        image_cropped,
                        in_range=(image.min(), np.percentile(image, 99.95)),
                    )

                sc_collage_row[:, :, ci] = image_cropped

            except Exception as e:
                print(f"Couldn't load {ch_pName+'/'+ch_fName}: {e}")
        ...
        if outline:
            ...
            outline_im = crop_cell.crop_single_cell_image(
                imD1, xCenter, yCenter, halfBoxSize
            )
        ...
        if info_columns:
            image_y_label = form_image_y_label_string(
                sc_df.loc[index, info_columns], info_columns
            )
