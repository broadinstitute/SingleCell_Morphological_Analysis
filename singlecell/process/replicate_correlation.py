...
#     corr_mat_df=df.set_index([pertColName,'Metadata_Plate']).loc[:,featColNames].T.corr()
    ...
    for u in highRepComp:
        ...
        repCorr=np.nanmean(repCorrPurtbs.values[np.triu_indices(repCorrPurtbs.shape[0], k = 1)])
        repCorrDf.loc[u,'RepCor']=repCorr

#         corr_rest=corr_mat_df.loc[u,not_u].mean().mean()
        corr_rest=corr_mat_df.loc[not_u,u].sample(repCorrPurtbs.shape[0]).mean().mean()

        rep_corr_ls.append(repCorr)
        null_corr_ls.append(corr_rest)
        
        
    rep_corr_ls = [rep_corr_ls for rep_corr_ls in rep_corr_ls if str(rep_corr_ls) != 'nan']
    null_corr_ls = [null_corr_ls for null_corr_ls in null_corr_ls if str(null_corr_ls) != 'nan']    
...
#         print(hist_bins)
        ...
        axes.legend(loc=2,fontsize=8);axes.set_title(title);
        axes.set_xlim(-1,1);
        plt.tight_layout() 
...
def weighted_replicate_enrichement_profiles(inDf,df_counts,pertColName,featColNames,plotEnabled,title=''):
...
#     featColNames=cpFeats_P_reduced
    ...
    # for u in ['GNRHR E90K-DMSO']:
        ...
        scaledvals_df=df.loc[df[pertColName]==u,featColNames].reset_index(drop=True)
        weights_df=df_counts.loc[df_counts[pertColName]==u,featColNames].reset_index(drop=True)

        u_not_u_df_idx=df.loc[df[pertColName].isin([u]+list(np.random.choice(not_u,scaledvals_df.shape[0])))].groupby(pertColName).sample(1).index
        scaledvals_df_rest=df.loc[u_not_u_df_idx,featColNames].reset_index(drop=True)
        weights_df_rest=df_counts.loc[u_not_u_df_idx,featColNames].reset_index(drop=True)
...
#         wcorr=matthews_corrcoef(vec0, vec1,sample_weight=weights_vec)
        wcorr=WeightedCorr.WeightedCorr(x=vec0,y=vec1,w=weights_vec)(method='pearson')
...
#         print(nS,[len(df2[pertColName].unique().tolist()),df1.shape[0]])
        for pa in range(len(repCorr)):
            ...
            dfTemp = pd.DataFrame({pertColName:[u],'R1':[Rep1],'R2':[Rep2],'Plate1':[Plate1],'Plate2':[Plate2],'cc':[repCorr[pa]]}) 
